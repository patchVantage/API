		...
		then
			...
			then
				...
				then
					if [ "${aws_type}" = "none" ]
                                        then
                                                message="$1 $2 Server on AWS instance-id=${instanceID}"
                                        else
                                                vcpu=`${PV_PATH} ec2-instance-vcpu --ec2-instance-type ${aws_type}`
                                                ram=`${PV_PATH} ec2-instance-memory --ec2-instance-type ${aws_type}`
                                                message="$1 $2 Server on AWS instance-type=${aws_type} VCPU=${vcpu} RAM=${ram}Gb"
                                        fi
					if [ "${WAIT}" = "yes" ]
					then
...
then
	...
	if (( $(echo "$available_memory > $sga_size" | bc -l) ))
		...
		then
			...
			echo "Examine View->Jobs in patchvantage.ai for progress - This Script will wait for completion"
