...
# 18c Database
RDBMS=ip-172-31-5-197.313.SALES
		...
		then
			...
			then
				...
				then
					if [ "${aws_type}" = "none" ]
                                        then
                                                message="$1 Server on Amazon instance-id=${instanceID}"
                                        else
                                                vcpu=`${PV_PATH} ec2-instance-vcpu --ec2-instance-type ${aws_type}`
                                                ram=`${PV_PATH} ec2-instance-memory --ec2-instance-type ${aws_type}`
                                                message="$1 Server on Amazon instance-type=${aws_type} VCPU=${vcpu} RAM=${ram}Gb"
                                        fi
					if [ "${WAIT}" = "yes" ]
					then
...
if [ "${set_name}" !=  "none" ]
then
	echo "Removing previous Backup ${set_name}" 
...
then
		...
		then
			...
			exit 1
                ...
                bold "There is not enough space on the server for this backup"
                exit 1
	fi
fi    
}
...
if [[ $? != 0  ]]; then exit 1 ;  fi
backup ${RDBMS}
pv display-backups| jq -r '.entities[] | "\(.SET_NAME)\t\(.INSTALLATION_DATE)"'
if [ "${WAIT_DB}" = "yes" ]
then
