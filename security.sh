...
#================================================================
#- IMPLEMENTATION
#-    version         security.sh  (www.patchvantage.com) 1.0.27
...
then
	...
	if [ ${retcode} -eq 0 ]
	then
		isEC2=`${PV_PATH} is-ec2 --environment $1`;retcode=$?
		if [ "${isEC2}" = "yes" -a ${retcode} -eq 0 ]
		then
			instanceID=`${PV_PATH} ec2-instance-id --environment $1`;retcode=$?
			if [ ${retcode} -eq 0 ]
			then
				aws_type=`${PV_PATH} ec2-instance-type --environment $1`;retcode=$?
				if [ ${retcode} -eq 0 ]
				then
 					if [ "${aws_type}" = "none" ]
                                        then
                                                message="$2 Server on Amazon instance-id=${instanceID}"
                                        else
                                                vcpu=`${PV_PATH} ec2-instance-vcpu --ec2-instance-type ${aws_type}`
                                                ram=`${PV_PATH} ec2-instance-memory --ec2-instance-type ${aws_type}`
                                                message="$2 Server on Amazon instance-type=${aws_type} VCPU=${vcpu} RAM=${ram}Gb"
                                        fi
					if [ "${WAIT}" = "yes" ]
					then
					...
					${PV_PATH} aws --operation $2 --instance ${instanceID} --wait ${WAIT} 
					return $?
				fi
			else
...
DIRECT)
	...
	then
		...
		json=`${PV_PATH} apply-security-patches --environment-names ${RDBMS} --force yes --delay ${DELAY}`
	else
		json=`${PV_PATH} apply-security-patches --environment-names ${RDBMS} --force no --delay ${DELAY}`
	fi
